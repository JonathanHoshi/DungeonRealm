...
namespace MagicArsenal
{
    public class MagicProjectileScript : MonoBehaviour
    {
        ...
        void FixedUpdate()
        {
            ...
            (bool hasHit, RaycastHit hit) = CheckHitObject(myTransform.position, rad, dir, dist);
            if (hasHit)
            {
        ...
        protected virtual void HandleHitEvent(RaycastHit hit) 
        {
        ...
        protected virtual (bool, RaycastHit) CheckHitObject(Vector3 position, float rad, Vector3 dir, float dist)
        {
            return (Physics.SphereCast(myTransform.position, rad, dir, out var hit, dist), hit);
        }
